...
async def predict(customer: CustomerData):
    ...
    input_tensor = torch.tensor([[customer.browsing_time]], dtype=torch.float32)
    
    # Make prediction
    with torch.no_grad():
        prediction = model(input_tensor)
        purchase_probability = prediction.item()
        purchase_decision = "WILL PURCHASE" if purchase_probability > 0.5 else "WON'T PURCHASE"
